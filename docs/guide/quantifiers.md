...
`check-loki` ships two quantifier-style entry points designed to plug into any
...
This is the most useful workflow check-loki enables: a CI failure
...
```ts
import { exists, Generators } from 'check-loki';
...
```ts
import { fromSeed, forAll, Generators } from 'check-loki';
