...
The whole API surface of `loki-check` is built on a handful of ideas. Once you
...
```ts
import { fromSeed, Generators } from 'loki-check';
...
construction time the recursive call has not been built yet. `loki-check`
